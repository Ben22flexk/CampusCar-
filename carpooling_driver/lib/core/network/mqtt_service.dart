...
class MqttService {
  ...
  Future<bool> subscribe(String topic, {MqttQos qos = MqttQos.atLeastOnce}) async {
    if (!_isConnected || _client == null) {
      developer.log(
        '❌ Cannot subscribe: not connected. isConnected: $_isConnected, client: ${_client != null}',
        name: 'MqttService',
      );
      return false;
    }
    ...
    try {
      ...
      if (_client!.connectionStatus?.state != MqttConnectionState.connected) {
        developer.log(
          '❌ Cannot subscribe: connection state is ${_client!.connectionStatus?.state}',
          name: 'MqttService',
        );
        return false;
      }
  ...
  void _onMessage(List<MqttReceivedMessage<MqttMessage>> messages) {
    ...
    for (final message in messages) {
      ...
      if (!_messageController.isClosed) {
        ...
        developer.log(
          '✅ Forwarded message to stream: $topic',
          name: 'MqttService',
        );
      } else {
        developer.log(
          '⚠️ Message controller is closed, cannot forward: $topic',
          name: 'MqttService',
        );
      }
    }
